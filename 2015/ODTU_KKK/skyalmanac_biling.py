...
#    Copyright (C) 2014  Mehmet Atakan Gürkan
#
...
from local_info_ODTUKKK_2015 import obs, obsTZ, utcTZ, year
from local_info_ODTUKKK_2015 import begin_day, begin_day_datetime, no_days
from local_info_ODTUKKK_2015 import first_sunday, first_sunday_datetime
from local_info_ODTUKKK_2015 import rising_bodies, transit_bodies, setting_bodies

mnt_names = ['sıfırıncı', 'Ocak', 'Şubat', 'Mart',
          'Nisan', 'Mayıs', 'Haziran', 'Temmuz',
          'Ağustos', 'Eylül', 'Ekim', 'Kasım', 'Aralık']
mnt_names_eng = ['zeroth', 'January', 'February', 'March',
          'April', 'May', 'June', 'July',
...
class Chart() :
    pass
chart = Chart()
chart.ULcorn = ephem.date(begin_day+4*ephem.hour) # at 4pm (in Turkey Sun does not set earlier than 4pm)
chart.URcorn = ephem.date(chart.ULcorn+16*ephem.hour) # 8 am next day (in Turkey Sun does not rise later than 8am)
...
sun = ephem.Sun()
# Twilights
# In Istanbul evening twilight always ends. This would not be true for
...
from local_info_ODTUKKK_2015 import venus
from local_info_ODTUKKK_2015 import jupiter, saturn, uranus, neptune
from local_info_ODTUKKK_2015 import m31, m42, m45
from local_info_ODTUKKK_2015 import arcturus, deneb, pollux, antares
...
for i, tlab in enumerate(['17', '18', '19', '20',
    ...
    if i== 7 :
        c.text(x, y1, 'midnight', [text.halign.center, text.valign.baseline])
    else :
...
c.text(x, y1, 'EVENING', [text.halign.center, text.valign.baseline])
x = chart.width*9.0/12.0
c.text(x, y2, 'SABAH', [text.halign.center, text.valign.baseline])
c.text(x, y1, 'MORNING', [text.halign.center, text.valign.baseline])
...
make_moon_key2_eng(c, chart, -2.0)
c.text(12., -1.25,
              r'{\footnotesize\sffamily M31: Andromeda Galaxy}',
              [text.halign.left,text.valign.baseline,color.cmyk.Gray])
c.text(12., -1.55,
              r'{\footnotesize\sffamily M42: Great Nebula in Orion}',
              [text.halign.left,text.valign.baseline,color.cmyk.Gray])
c.text(12., -1.85,
              r'{\footnotesize\sffamily M45: Pleiades (Seven Sisters)}',
              [text.halign.left,text.valign.baseline,color.cmyk.Gray])

c.text(17., -1.25,
              r'{\footnotesize\sffamily doğuyor: rising}',
              [text.halign.left,text.valign.baseline,color.cmyk.Gray])
c.text(17., -1.55,
              r'{\footnotesize\sffamily meridyende: transit}',
              [text.halign.left,text.valign.baseline,color.cmyk.Gray])
c.text(17., -1.85,
              r'{\footnotesize\sffamily batıyor: setting}',
              [text.halign.left,text.valign.baseline,color.cmyk.Gray])

make_moon_key2(c, chart, chart.height+1.2)
c.text(12., chart.height+1.2+1.8-1.1,
              r'{\footnotesize\sffamily M31: Andromeda Gökadası}',
              [text.halign.left,text.valign.baseline,color.cmyk.Gray])
c.text(12., chart.height+1.2+1.8-1.4,
              r'{\footnotesize\sffamily M42: Avcı Bulutsusu}',
              [text.halign.left,text.valign.baseline,color.cmyk.Gray])
c.text(12., chart.height+1.2+1.8-1.7,
              r'{\footnotesize\sffamily M45: Ülker (Yedi Kızkardeşler)}',
              [text.halign.left,text.valign.baseline,color.cmyk.Gray])
...
c.writePDFfile("almanac_%d_ODTUKKK" % (year))
#c.writeEPSfile("almanac_%d_ODTUKKK" % (year))
